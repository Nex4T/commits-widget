    ...
    success: function(response, textStatus, jqXHR) {
      ...
      $.each(data, function(i, commit){
        ...
        if( commit.sha && commit.commit && commit.commit.message && commit.commit.author && commit.commit.committer && commit.commit.committer.date ) {
          ...
          $('<span class="commit-meta">by <span class="committer-name">' + commit.commit.author.name + '</span> <br> <span class="commit-time">' + $.timeago(commit.commit.committer.date) + '</span></span>').appendTo( $right );
